...
void triangle(mat<4, 3, float> & clipc, IShader & shader, FrameTile & image)
{
    ...
    Vec2f clampTopLeft(static_cast<float>(image.get_left()), static_cast<float>(image.get_top()));
    Vec2f clampBottomRight(static_cast<float>(image.get_right() - 1), static_cast<float>(image.get_bottom() - 1));
    ...
    for (P.x = static_cast<int>(bboxmin.x); P.x <= bboxmax.x; ++P.x)
    {
        for (P.y = static_cast<int>(bboxmin.y); P.y <= bboxmax.y; ++P.y)
        {
            ...
            if (bc_screen.x < 0 || bc_screen.y < 0 || bc_screen.z<0 || image.get_z(P.x, P.y)>frag_depth)
            {
                continue;
            }
